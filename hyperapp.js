...
class VNode {
    ...
    constructor(tag, children, props = {}, text = "") {
        if (nodes.indexOf(tag) == -1) throw new Error("Tag " + tag + " isn't a valid HTML node");
    ...
    callEvent(eventName, args = []) {
        ...
        if(this.#props.events && this.#props.events[eventName]) this.#props.events[eventName](e, Date.now(), this);
    }
    ...
    removeChildren(q) {
        ...
        if (index != undefined) this.#children = this.#children.filter((_, i) => i != index);
    ...
    render() {
        ...
        if (this.#props.classes) this.#props.classes.forEach(cls => el.classList.add(cls))
        let clonedProps = {};
        Object.keys(this.#props).forEach((prop, i) => {
            if (prop != "events")
...
class VDOM {
    ...
    render() {
        ...
        this.#subscription.find(el => (["hyperappUpdate", "update", "hyperUpdate"].indexOf(el.name) != -1))?.invoke(this, Date.now());
        let gen_els = this.#elements(this.#state);
        if (!Array.isArray(gen_els)) gen_els = [gen_els];
        gen_els.forEach(el => {
            el.vdom = this;
            ["hyperappUpdate", "update", "hyperUpdate"].forEach(e=>el.callEvent(e, [this, Date.now()]))
        })
    ...
    get children() { return this.#elements }
    
    get parentElement() { return this.#parentElement }
    
    set children(val) { this.#elements = val; }

    set parentElement(val) { this.#parentElement = val; }
}
...
const h = (tag, children, props = {}, text = "") => new VNode(tag, children, props, text);

function addEvent(vdom, el, e, vnode = undefined) {
    print(vdom, el, e, vnode);
        addEventListener(e.name, (...args) => { e.invoke(new HEvent(e, args, vnode)); vdom.render()})
}

function app(options) {
    let vdom = new VDOM(options.node||document.body, options.elements||[], options.subscriptions||[], options.init || {})
...
function route(routes) {
    if (/^((https{0,1}){0,1}(file){0,1}):\/{2,3}([a-zA-Z\.\-_]+(:[a-zA-Z\.\-_]+){0,1}@)*[a-zA-Z\.\-_]+(\/[a-zA-Z\.\-_]*)+([\?][=a-zA-Z\-._\+%]+(&[=a-zA-Z\-._\+%]+)*)*$/g.test(location.href)) {
        let domain = location.href
          .replace(location.protocol + "//" + location.host, "")
            .match(/\/([a-zA-Z_\-.]+\/{0,1})*/)[0];
        routes.forEach(route =>{if (route.path == domain) route.app.render(); })
    }
    else {
